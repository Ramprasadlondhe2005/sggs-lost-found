      ...
      <div className="hidden md:flex md:w-1/2 relative overflow-hidden items-center justify-center bg-muted">
        ...
        <div className="relative z-20 text-center p-8 max-w-lg">
          <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} transition={{ duration: 0.8, delay: 0.2 }}>
            ...
            <h2 className="text-4xl font-bold text-white mb-4 drop-shadow-md">Admin Portal</h2>
            <p className="text-lg text-white/90 drop-shadow">Manage the entire SGGS Lost and Found operation infrastructure safely and securely.</p>
          </motion.div>
        </div>
      </div>
        ...
        >
          ...
          <div className="bg-card/80 backdrop-blur-xl rounded-2xl border border-border/50 p-8 shadow-2xl relative overflow-hidden">
            ...
            <div className="flex items-center gap-3 relative z-10">
              <div className="p-2 bg-primary/10 rounded-lg"><ShieldCheck className="w-6 h-6 text-primary" /></div>
              <h2 className="text-2xl font-bold text-foreground">Admin Access</h2>
            </div>
            <p className="text-sm text-muted-foreground mt-2 relative z-10">Restricted access — administrators only</p>
            ...
            <form onSubmit={handleSubmit} className="mt-6 space-y-5 relative z-10">
              <div>
                ...
                <input required type="email" value={email} onChange={e => setEmail(e.target.value)} placeholder="admin@sggs.ac.in" className="mt-1.5 w-full rounded-xl border border-input bg-background/50 backdrop-blur-sm px-4 py-3 text-sm text-foreground focus:ring-2 focus:ring-primary/30 focus:border-primary outline-none transition-all" />
              </div>
              <div>
              ...
              <div>
                ...
                <input required value={twoFA} onChange={e => setTwoFA(e.target.value)} placeholder="000000" maxLength={6} className="mt-1.5 w-full rounded-xl border border-input bg-background/50 backdrop-blur-sm px-4 py-3 text-sm text-center font-mono text-foreground focus:ring-2 focus:ring-primary/30 focus:border-primary outline-none tracking-[0.5em] transition-all" />
              </div>
              <button disabled={!email || !password || !twoFA} type="submit" className="w-full bg-primary text-primary-foreground py-3 rounded-xl text-sm font-semibold hover:bg-primary-hover transition-all shadow-md hover:shadow-lg hover:-translate-y-0.5 active:translate-y-0 disabled:opacity-50 disabled:hover:-translate-y-0 mt-2">
                Authorize Session
              </button>
            </form>
            ...
            <div className="mt-8 pt-6 border-t border-border/50 relative z-10">
              <p className="text-sm text-muted-foreground text-center">
                Return to <Link to="/" className="text-primary hover:text-primary-hover font-semibold transition-colors">Homepage</Link>
              </p>
            </div>
          </div>
        </motion.div>
